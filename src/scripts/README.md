...
#### 4. Реальный запуск с лимитом:
```bash
npx tsx src/scripts/backfillMFEMAE.ts --limit 10
```
...
- **Rate limiting**: Binance API имеет лимит 1200 запросов/минуту
- Скрипт делает паузу 500ms между сигналами для соблюдения лимита
- Для сигналов длительностью >1000 минут (~16.6 часов) делается несколько запросов
